...
import com.pr_asistencia.asistencia_auto.worker.AttendanceWorker
...
class HomeActivity : AppCompatActivity() {
    ...
    private fun guardarConfiguracion()
    {
        ...
        prefs.edit {
            putString(
                "horaEntrada",
                txtHoraEntrada.text.toString()
            ).putString(
                "horaSalida",
                txtHoraSalida.text.toString()
            ).putBoolean(
                "automatico",
                switchAutomatico.isChecked
            ).putBoolean(
                "activo",
                switchActivo.isChecked
            ).putBoolean(
                "lunes",
                checkLunes.isChecked
            ).putBoolean(
                "martes",
                checkMartes.isChecked
            ).putBoolean(
                "miercoles",
                checkMiercoles.isChecked
            ).putBoolean(
                "jueves",
                checkJueves.isChecked
            ).putBoolean(
                "viernes",
                checkViernes.isChecked
            )
        }

        val securePrefs = App.instance.securePrefs()

        val user = securePrefs.getString(
                "user",
                ""
            ) ?: ""
        ...
        AlarmHelper.programarAlarma(
            this,
            entradaSplit[0].toInt(),
            entradaSplit[1].toInt(),
            100
        )
